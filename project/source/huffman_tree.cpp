...
void HuffmanTree::print() const {   // need to implement this function 
  ...
  HuffmanNode* stackNodes[1000]; //stores nodes in tree
  string stackCodes[1000]; // stores codes for corresponding tree
  int top = -1; // pointer index

  //traversing from root 
  top++;
  stackNodes[top] = this -> root;
  stackCodes[top] = ""; //start iwth empty

  //while loop for depth first tree
  while (top >= 0){
    HuffmanNode* current = stackNodes[top]; // start element at top of stack
    string code = stackCodes[top];
    top--;

    //skip if the node is nullptr
    if (current == nullptr){
      continue;
    }

    // if child node is there and contains data store the code for that data
    if (current -> left == nullptr && current -> right == nullptr){
      ...
      if (current -> right != nullptr){
        ...
        stackCodes[top] = code + "1"; // add a 1 with the right push onto stack
      }
      // push left
      if (current -> left != nullptr){
        top++;
        stackNodes[top] = current -> left;
        stackCodes[top] = code + "0"; // add 0 for the left stack
      }
    }
  }
